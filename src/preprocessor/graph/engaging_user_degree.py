...
def engaging_user_degree(raw_data, features, auxiliaries):
    ...
    sdf_raw_data_index = sdf_raw_data.selectExpr(
        "tweet_id", "engaging_user_id AS id"
    )

    sdf_engaging_user_in_degree = sdf_raw_data_index.join(sdf_user_in_degree, on="id", how='left_outer')
    sdf_engaging_user_out_degree = sdf_raw_data_index.join(sdf_user_out_degree, on="id", how='left_outer')
    ...
    ks_engaging_user_in_degree_with_index = kdf_engaging_user_in_degree.set_index(["tweet_id", "engaging_user_id"])["inDegree"]
    ks_engaging_user_out_degree_with_index = kdf_engaging_user_out_degree.set_index(["tweet_id", "engaging_user_id"])["outDegree"]
